from dash import Dash, dcc, _dash_renderer, callback, Output, Input
    ...
    children = [
        dbc.Container(
            children = [
                dbc.Row(
                    dmc.MantineProvider(
                        dmc.Text("Sales Analysis Dashboard", size="xl", c = "white", style={"fontSize": 40})
                    ),
                ),
                dbc.Row(
                    [
...
if __name__ == '__main__':
    app.run(debug = True, use_reloader = True, dev_tools_hot_reload_interval = 1)